		...
		PARSER: for (int i = 0; i < rawArgs.length; i++)
		{
			...
			if (arguments.get(i) instanceof ContinuousArgument)
			{
				...
				for (int j = i; j < rawArgs.length; j++)
				{
					...
					{
						arguments.add(j + 1, new ProcessedArgument<Object>(arguments.get(i).getName(), arguments.get(i).getType(), arg.getName(), formatString(rawArgs[j])));
					}
					else if (arguments.get(i).getType() == ParamType.CHAR)
					{
						arguments.add(j + 1, new ProcessedArgument<Object>(arguments.get(i).getName(), arguments.get(i).getType(), arg.getName(), rawArgs[j]));
					}
					else if (arguments.get(i).getType() == ParamType.BOOLEAN)
					{
						arguments.add(j + 1, new ProcessedArgument<Object>(arguments.get(i).getName(), arguments.get(i).getType(), arg.getName(), formatToBoolean(rawArgs[j])));
					}
					else
					{
						arguments.add(j + 1, new ProcessedArgument<Object>(arguments.get(i).getName(), arguments.get(i).getType(), arg.getName(), formatToNumber(rawArgs[j])));
					}
				}
			...
			case BOOLEAN:
				if (!rawArgs[i].contains(":"))
				{
					...
					{
						processedArguments.add(new ProcessedArgument<Boolean>(arguments.get(i).getName(), arguments.get(i).getType(), rawArgs[i], formatToBoolean(rawArgs[i])));
					}
					else
					{
			...
			case CHAR:
				...
				else if (rawArgs[i].contains(":"))
				{
					...
					processedArguments.add(new ProcessedArgument<Character>(arguments.get(i).getName(), arguments.get(i).getType(), tag, cValue));
				}
				else
				{
					processedArguments.add(new ProcessedArgument<Character>(arguments.get(i).getName(), arguments.get(i).getType(), rawArgs[i], rawArgs[i].charAt(0)));
				}
				break;
			case BYTE:
			case DOUBLE:
				...
				try
				{
					if (tm.find() && !rawArgs[i].contains(":"))
					{
						...
						if (!rawArgs[i].contains("%"))
						{
							...
							processedArguments.add(new ProcessedArgument<Number>(arguments.get(i).getName(), arguments.get(i).getType(), rawArgs[i], NumberFormat.getInstance().parse(rawArgs[i])));
							break;
						...
						else
						{
							if (arguments.get(i).hasRange())
							{
								...
								rawArgs[i] = Integer.toString(MathUtils.getPercentageValue(percentage, arguments.get(i).getMin(), arguments.get(i).getMax()));
								processedArguments.add(new ProcessedArgument<Number>(arguments.get(i).getName(), arguments.get(i).getType(), rawArgs[i], NumberFormat.getInstance().parse(rawArgs[i])));
								break;
							}
						}
					...
					{
						command.shutdown(obj, Status.FAILED, "Given parameter (" + rawArgs[i] + ") is not a valid integer value!");
					}
				}
			...
			case STRING:
				...
				if (command.arguments.size() == 1 && command.getOptArgCount() == 0 && !(command.arguments.get(0) instanceof ContinuousArgument))
				{
					...
					processedArguments.add(new ProcessedArgument<String>(arguments.get(i).getName(), arguments.get(i).getType(), concat, concat));
					break PARSER;
				}
				if (!rawArgs[i].contains(":"))
				{
					...
					{
						command.shutdown(obj, Status.FAILED, "A string parameter (" + rawArgs[i] + ") was given that exceed its maximum value of " + arguments.get(i).getMax() + "!");
						break;
					}

					rawArgs[i] = formatString(rawArgs[i]);
					processedArguments.add(new ProcessedArgument<String>(arguments.get(i).getName(), arguments.get(i).getType(), rawArgs[i], rawArgs[i]));
				}
				else
				{
					String[] split = rawArgs[i].split(":", 2);
			...
			case GROUP:
				...
				ProcessedGroupArgument group = new ProcessedGroupArgument(arguments.get(i).getName(), arguments.get(i).getType(), rawArgs, null);
