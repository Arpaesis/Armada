 ...
 *
 * @param <T>
 *            The type that command manager will expect to be used in command execution.
 * @param <R>
 *            The type that the command manager will expected to be returned from the executed command.
 */
public final class CommandManager<T, R>
{
				...
				for (String alias : com.getValue().getAliases())
				{
					if (command.getAliases() != null)
					{
						for (String toCheck : command.getAliases())
						{
							...
							{
								System.err.println(
										"Warning: command (" + command.getName() + ") has an alias (" + alias + ") that is already used by another command (" + com.getValue().getName() + ")!");
							}
						}
					}
				}
	...
	/**
	 * Removes the specified command from the registry. Commands are unregistered by name.
	 * 
	 * @param command
	...
	/**
	 * Gets a command object with the specified primary name. Does not accept aliases.
	 * 
	 * @param name
	...
	/**
	 * The method used by the command manager to execute a [@link Command}. Also fires even if the input is not a command.
	 * 
	 * @param obj
	 *            The object to be used in the command's execution.
	 * @param in
	 *            The raw input to be processed by the {@link Parser}.
	 ...
	 * 
	 * @param obj
	 *            The object to be used in the response.
	 * @param in
	 *            The input to be used in the response execution.
	 */
	 ...
	 * 
	 * @param isEnabled
	 *            Whether or not the command manager should be enabled.
	 */
